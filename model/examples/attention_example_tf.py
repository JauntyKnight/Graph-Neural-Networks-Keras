...
node_representation_layer = ApplyOverBatch(
    MultiHeadAttention(256, 4, activation=ops.relu, name="attention")
)([node_representation_layer, edge_list_inputs_og])
...
for i in range(1):
    node_representation_layer = ApplyOverBatch(
        MultiHeadAttention(256, 2, activation=ops.relu, name=f"attention{i+1}")
    )([node_representation_layer, edge_list_inputs_og])
...
model = keras.Model(
    inputs=[edge_list_inputs_og, node_inputs_og],
    outputs=outputs,
)
